...
_REDIRECT_TEMPLATE = """\
lockedin: '{cmd}' runs inside Claude Code — the skill drives the LLM
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"lockedin {__version__}")
    ...
    p_init.add_argument("--vault", help="vault path (default ~/Documents/LockedIn/)")
    ...
    grp.add_argument("--setup-hud", action="store_true",
                     help="wire lockedin HUD into Claude Code's statusLine")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "migrate":
        from lockedin.commands.migrate import migrate
