...
- **자연어로 활성화.** 외울 명령은 없습니다. Claude Code에 평소처럼 말 걸면 LockedIn이 알아서 반응합니다.
- **하나의 vault, 네 가지 산출물.** 영문 이력서, 한국 자소서, 면접 답변, 새 프로젝트 아이디어가 모두 같은 경험 출처에서 나옵니다.
- **두 turn writer/reviewer로 검증된 출력.** 작성과 채점을 별개의 Claude turn으로 분리해 자기 평가의 편향(약 1점 부풀림)을 막습니다. 매 산출물마다 5차원 JSON 점수가 함께 출력되어 제출 전에 품질이 보입니다.
- **API key 없이 구독으로 동작.** 이미 가진 Claude Code 구독 위에서 모든 추론이 돌아갑니다. 별도 API 키나 결제 없습니다.
- **완전한 사용자 소유.** vault는 `~/Documents/LockedIn/`의 평범한 마크다운입니다. Obsidian과 호환되며 다른 도구로 가져갈 수 있고, 외부로 데이터를 전송하지 않습니다.
...
vault 루트인 `~/Documents/LockedIn/`에는 `EXPERIENCE.md`라는 마스터 파일이 자동으로 갱신됩니다. 사람, 회사, 역할, 프로젝트처럼 타입별로 폴더가 나뉘어 있는데, 매번 폴더를 열어보지 않고 전체 내용을 한 번에 훑을 수 있습니다. 사용자가 직접 마크다운을 편집한 뒤에 마스터 파일이 stale해 보이면 `lockedin refresh`로 다시 모을 수 있습니다.
