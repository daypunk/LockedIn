...
vault 루트인 `~/Documents/LockedIn/`에는 `EXPERIENCE.md`라는 마스터 파일이 자동으로 갱신됩니다. 사람, 회사, 역할, 프로젝트처럼 타입별로 폴더가 나뉘어 있는데, 매번 폴더를 열어보지 않고 이 파일 하나만 펼치면 vault 전체 내용을 한 번에 훑을 수 있습니다. 사용자가 직접 마크다운을 편집한 뒤에 마스터 파일이 stale해 보이면 `lockedin refresh`로 다시 모을 수 있습니다.
...
| `lockedin-render-interview` | 면접 답변 렌더러. STAR 또는 PAR 구조로, 한 문단당 한 경험을 명시하고 그 사이에 명시적 전환 문장을 둡니다. 다섯 차원으로 평가합니다. clarity, evidence density, persona fit, conciseness, tone. v1.1은 프롬프트와 rubric 스캐폴드까지 ship되어 있고, 외부 코퍼스와의 cross-source calibration은 v1.2 타겟입니다. |
| `lockedin-render-ideas` | vault를 바탕으로 다음 프로젝트, 사이드 프로젝트, 커리어 전환 아이디어를 3~5개 surface합니다. 각 아이디어는 한 문단입니다. 한 줄 피치 + 실제 vault entity를 인용한 근거. 다섯 차원으로 평가합니다. feasibility, novelty, evidence ground, scope match, motivation alignment. v1.1은 프롬프트와 rubric 스캐폴드까지 ship되어 있고, calibration은 v1.2 타겟입니다. |
