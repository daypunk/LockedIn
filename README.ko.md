...
- **사전 점검 audit.** vault를 만들기 전이라도 아무 이력서나 던져서 동일한 calibrated 점수를 받아볼 수 있습니다. 설치도 가입도 필요 없습니다.
...
| `lockedin-audit` | 캘리브레이션된 사전 점검 스코어러. 임의의 이력서 문서를 받아 5차원 rubric 점수와 banned phrase, 약한 동사 hit 목록을 반환합니다. 선택적인 refinement 패스는 점수 상승폭까지 정량화합니다. `render-resume-en`과 `render-jaso`의 rubric을 재사용하므로 calibration 중복이 없습니다. |
