...
| 仕事の瞬間をキャプチャ | `/lockedin-capture` | 「これを保存して」「これを記録して」「これを追跡して」などのキャプチャ意図を構造化されたエントリに変換します。writer/reviewer パターン、5 次元採点 (スキーマ準拠、エッジ完全性、フィールド具体性、意味精度、重複検出)。類似候補があれば surface してユーザーに尋ね、統合か別個かを決めてもらいます。沈黙して統合したり、沈黙して重複を作ったりしません。 |
