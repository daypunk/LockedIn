...
他のツールはあなたの仕事の外にいます。LockedIn は仕事の中にいます。コードの一行をマージした瞬間、会議の結論が決まった瞬間、新しい設計判断を下した瞬間が、構造化された経歴として保存されます。流れは途切れません。

一度保存された経歴は成果物につながります。半年後にレジュメが必要になったとき、その半年間の実際の経歴がそこにそのまま残っています。誰かのデータベースではなく、あなたのファイルシステムに Markdown として蓄積されているので、どのツールにも持ち出せますし、どの成果物にも再利用できます。
...
| 仕事の瞬間をキャプチャ | `/lockedin-capture` | 「これを保存して」「これを記録して」「これを追跡して」などのキャプチャ意図を構造化されたエントリに変換します。writer/reviewer パターン、5 次元(スキーマ準拠、エッジ完全性、フィールド具体性、意味精度、重複検出)で採点。類似候補があれば surface してユーザーに尋ね、統合か別個かを決めてもらいます。沈黙して統合したり、沈黙して重複を作ったりしません。 |
| 英文レジュメを書く | `/lockedin-render-resume-en` | 経歴から引いてペルソナに合わせた英文レジュメを書きます。内蔵ペルソナは 10 種 (シニア IC、ミッドレベル、PM、バックエンド、フロントエンド、モバイル、データエンジニア、ML エンジニア、デザイナー、マーケター) で、他の職種でも動きます。5 次元(指標密度、動詞品質、構造、お決まり表現、ペルソナ適合性)で採点し、内蔵外の職種ではペルソナ次元のみ控えめに評価されます。 |
...
| 面接回答を起草する | `/lockedin-render-interview` | 会社、職種、質問を渡すと STAR または PAR の構造で答えます。一段落一経験、段落間には明示的な遷移文を入れて面接官が追いやすくします。5 次元(明瞭性、根拠、ペルソナ適合性、簡潔性、トーン)で採点します。 |
| 次のアイデアを提示する | `/lockedin-render-ideas` | あなたの経歴を読んで次に取りうる方向を 3 から 5 個提案します。各アイデアは一段落のピッチと、なぜそれがあなたに合うかを示す経歴の引用です。5 次元(実現可能性、新規性、根拠、スコープ整合、動機整合)で採点します。 |
