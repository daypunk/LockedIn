...
LockedIn は、あなたの Claude Code セッションの中にいます。コードを書いている時、ドキュメントを作成している時、意味のある仕事を構造化された経歴として保存してください。後にレジュメ、韓国語自己紹介書、面接回答、プロジェクトのアイデアが必要になれば、LockedIn がその構造化された経歴をもとに書き起こします。
...
覚えるコマンドもなく、別タブを開く必要もありません。Claude Code の中で自然に始めてください。
...
仕事の途中で、たった今起きたことをキャプチャしたいとき:

- "このコミットをプロジェクトのハイライトとして保存して"
- "ミーティングが終わったから記録して"
- "今学んだことを追加して"
...
ほとんどの経歴整理ツールは、あなたが今やっていることを止めて、別のところにログインして、大事だったことを思い出してほしいと言います。その頃にはすでに半分は頭から失われています。

LockedIn は仕事が起きている場所にいます。Claude Code でコードを書いている時、ドキュメントを作成している時、PR をレビューしている時、「これを LockedIn に保存して」と言えば、その瞬間が構造化された Markdown ファイルとしてローカルに蓄積されます。経歴を整理するために第二のツールを使ったり、コンテキストをスイッチしたりする必要はありません。

ファイルはあなたのものです。どのエディタでも開けますし、どのツールにも持ち出せます。レジュメ、カバーレター、面接回答が必要になれば、LockedIn が構造化された経歴とスキルをもとに書いてくれます。

## 特徴

- **覚えるコマンドはありません。** 話すように頼めば LockedIn が適切なスキルを選びます。
- **仕事中にキャプチャします。** Claude Code セッションの一瞬を、コンテキストスイッチなしで構造化された経歴として保存します。
- **成果物を採点します。** 書いた Claude とは別の Claude が、ドメインリサーチに基づいたルーブリックで採点します。
- **API キー不要。** 既存の Claude Code サブスクリプションで動きます。

## 仕組み

**1. 経歴が構造化されます。** 既存のレジュメを投げるか、短いインタビューに答えるか、仕事の途中で意味のある瞬間をキャプチャしてください。LockedIn が `~/Documents/LockedIn/` の中に 15 種類の型付き Markdown ファイルとして整理します。
...
| 韓国語自己紹介書を書く | `/lockedin-render-jaso` | 会社名と質問を渡すと、あなたの経歴を引用して答えを書きます。韓国語 5 次元 (結論先行、構造化、具体性、表現、適合性) で採点し、複数ソースで相互検証されたお決まり表現を避けます。 |
