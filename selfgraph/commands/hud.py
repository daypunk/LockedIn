"""selfgraph hud — emit a one-line statusLine snippet for Claude Code.
...
def _render(vault: Path, color: bool) -> str:
    ...
    # 1. service + version
    parts.append(_ansi(f"selfgraph {__version__}", C_CYAN, color))
...
def hud(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="selfgraph hud")
