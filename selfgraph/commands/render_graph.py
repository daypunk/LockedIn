"""selfgraph render graph — produce graph.json and an interactive graph.html.

Pure-CLI utility. The interactive HTML uses the bundled ``force-graph``
UMD library (vendored under ``plugins/selfgraph/scripts/vendor/``) so the
...
from selfgraph.storage.graph import write_graph_json
...
def _vendor_bundle() -> str:
    ...
    vendor = repo_root / "plugins" / "selfgraph" / "scripts" / "vendor" / VENDOR_BUNDLE_NAME
...
def _interactive_html(graph: dict, bundle_js: str) -> str:
    ...
    return (
        ...
        "<body>\n"
        + (
            f'  <div id="meta"><strong>selfgraph</strong> — {n_nodes} nodes · {n_edges} edges</div>\n'
            '  <div id="help">drag to pan · scroll to zoom · click a node to focus</div>\n'
            '  <div id="graph"></div>\n'
            if n_nodes > 0
            else '  <div class="empty">vault is empty — run `/selfgraph init` to seed it.</div>\n'
        )
        + "  <script>\n"
        + bundle_js
        + "\n  </script>\n"
...
def _placeholder_html(graph: dict) -> str:
    ...
    return (
        "<!doctype html>\n"
        '<html lang="en">\n<head><meta charset="utf-8"><title>selfgraph — graph (fallback)</title>\n'
