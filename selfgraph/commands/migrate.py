"""selfgraph migrate — upgrade vault frontmatter to the latest schema version.
...
from selfgraph.ontology import SCHEMA_VERSION
from selfgraph.storage.notes import read_entity, write_entity
...
def migrate(vault_arg: str | None = None, dry_run: bool = False) -> int:
    ...
    if skipped:
        print(f"  ({skipped} unreadable notes skipped, run `selfgraph validate` to surface them)")
    return 0
