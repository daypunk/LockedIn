"""selfgraph install — fallback for users who prefer not to use plugin marketplace.
...
MANIFEST_NAME = ".selfgraph-manifest.json"

# Stable, version-independent location for the HUD script that statusLine
# points at. Kept under ~/.claude/selfgraph/ so a plugin upgrade (which
...
def _skill_source() -> Path:
    ...
    candidates = [
        repo_root / "plugins" / "selfgraph" / "skills",
        repo_root / "selfgraph" / "skill",  # legacy fallback
    ]
...
def install_check(target: str | None = None) -> int:
    ...
    main = dest_root / "selfgraph" / "SKILL.md"
    if not main.exists():
        print(f"selfgraph skill NOT installed under: {dest_root}")
        print(
            "Recommended: install via Claude Code plugin marketplace:\n"
            "  /plugin marketplace add daypunk/selfgraph\n"
            "  /plugin install selfgraph@selfgraph\n"
            "Manual fallback:\n"
            "  selfgraph install --auto-register"
        )
        return 1

    manifest_path = dest_root / "selfgraph" / MANIFEST_NAME
    version = "?"
    files_count = 0
    ...
    skills = sorted(
        ...
        if p.is_dir() and p.name.startswith("selfgraph") and (p / "SKILL.md").exists()
    )
    print(f"selfgraph installed at: {dest_root}")
...
def install_auto_register(target: str | None = None, *, force: bool = False) -> int:
    ...
    main_skill = dest_root / "selfgraph" / "SKILL.md"
    if main_skill.exists() and not force:
        print(f"already installed under {dest_root}.")
        print("use `selfgraph install --upgrade` (hash-aware) or `--force` to overwrite.")
        return 1
    ...
    (dest_root / "selfgraph" / MANIFEST_NAME).write_text(
        json.dumps(manifest, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    names = ", ".join(s.name for s in skill_dirs)
    print(f"installed selfgraph v{__version__} ({len(skill_dirs)} skills: {names}) to {dest_root}.")
    return 0
...
def install_upgrade(target: str | None = None, *, force: bool = False) -> int:
    ...
    print(f"upgraded selfgraph at {dest_root} to v{__version__}.")
    return 0
...
def _hud_install_dir() -> Path:
    return _claude_config_dir() / "selfgraph"
...
def _hud_source_script() -> Path:
    ...
    here = Path(__file__).resolve()
    repo_root = here.parent.parent.parent
    return repo_root / "plugins" / "selfgraph" / "scripts" / "hud.py"


def install_setup_hud(target: str | None = None, *, force: bool = False) -> int:
    """Wire selfgraph HUD into Claude Code's statusLine.

    Idempotent. Writes a self-contained HUD script to a stable location
    (``~/.claude/selfgraph/hud.py``) and updates ``settings.json`` so
    Claude Code calls it. Existing non-selfgraph statusLine configs are
    ...
    already_ours = (
        ...
        and "selfgraph" in existing["command"]
    )
    ...
    if existing and not already_ours and not force:
        ...
        print(
            f"selfgraph HUD: existing statusLine backed up to {backup_path}"
        )

    settings["statusLine"] = new_status_line
    _write_settings(settings)

    print(f"selfgraph HUD wired into {_settings_file()}.")
...
def install_remove_hud(target: str | None = None) -> int:
    ...
    is_ours = (
        ...
        and "selfgraph" in existing.get("command", "")
    )
    ...
    if is_ours:
        if backup_path.exists():
            try:
                ...
                print(f"selfgraph HUD: restored previous statusLine from {backup_path}.")
            except json.JSONDecodeError:
                settings.pop("statusLine", None)
                print("selfgraph HUD: backup unreadable; removed statusLine.")
        else:
            settings.pop("statusLine", None)
            print("selfgraph HUD: removed statusLine entry.")
        _write_settings(settings)
    else:
        print("selfgraph HUD: statusLine is not selfgraph's; nothing to undo.")

    script_path = install_dir / HUD_SCRIPT_BASENAME
    if script_path.exists():
        script_path.unlink()
        print(f"selfgraph HUD: removed {script_path}.")
    if backup_path.exists():
        backup_path.unlink()
...
def install_uninstall(target: str | None = None) -> int:
    ...
    for child in dest_root.iterdir() if dest_root.exists() else []:
        if child.is_dir() and child.name.startswith("selfgraph") and (child / "SKILL.md").exists():
            shutil.rmtree(child)
            removed.append(child.name)
    if not removed:
        print(f"nothing to uninstall under {dest_root}.")
        return 0
    print(f"uninstalled selfgraph skills from {dest_root}: {', '.join(sorted(removed))}.")
    return 0
