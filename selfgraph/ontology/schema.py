"""Canonical ontology schema for selfgraph (v0.2).
...
- ``SCHEMA_VERSION`` → bumped on breaking changes; surfaced via
  ``selfgraph validate`` so vaults pinned to an older version can be
  migrated explicitly.
...
``selfgraph/commands/validate.py`` so authoring stays cheap (you can
...
# `selfgraph/commands/validate.py` reads it; renderers query it; tests fix
# against it.
...
class Entity:
    ...
    ``selfgraph validate``. Construction here only checks that ``type``
