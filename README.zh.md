...
| 捕获工作瞬间 | `/lockedin-capture` | 把"存这个"、"记一下这个"、"跟踪这个"等捕获意图转换为结构化条目。writer/reviewer 模式,5 维(schema 准守、edge 完整度、字段具体性、语义准确度、重复检测)评分。发现相似候选时会浮现给你,由你决定合并还是保留;不会静默合并,也不会静默创建重复。 |
| 撰写英文简历 | `/lockedin-render-resume-en` | 从经历调取内容,按 persona 撰写英文简历。内置 10 种 persona(senior IC、mid 级、PM、后端、前端、移动端、数据工程、ML 工程、设计、市场),其他岗位也能用。5 维(指标密度、动词质量、结构、banned 表达、persona 适配)评分,内置之外的岗位只在 persona 维度上略保守。 |
| 撰写韩文求职信 | `/lockedin-render-jaso` | 给定公司和题目,会引用你的经历写出答案。按韩文 5 维(结论先行、结构化、具体性、表达、适配)评分,并避开跨源验证过的禁用表达。 |
| 起草面试回答 | `/lockedin-render-interview` | 给定公司、岗位和问题,以 STAR 或 PAR 结构作答。每段只放一个经验,段落间放显式过渡句让面试官更容易跟上。5 维(清晰度、证据、persona 适配、简洁度、语气)评分。 |
| 提出下一步想法 | `/lockedin-render-ideas` | 读你的经历,提出 3 到 5 个可走方向。每个想法一段。一句 pitch,加上引用你匹配经历作为依据。5 维(可行性、新颖度、证据基础、范围匹配、动机对齐)评分。 |
