#!/usr/bin/env python3
"""selfgraph HUD — self-contained statusLine emitter.

Output (one line, exit 0):
    selfgraph X.Y.Z │ 5h:NN% · wk:NN% │ vault: Nn · Me
...
week). Override with SELFGRAPH_HUD_5H_LIMIT and SELFGRAPH_HUD_WK_LIMIT.

Color is on by default. Disable via NO_COLOR or SELFGRAPH_HUD_COLOR=0.

The script defers to the installed selfgraph package if available so
...
On any unexpected error, falls back to printing 'selfgraph' and exit 0.
"""
...
try:
    from selfgraph.commands.hud import hud as _package_hud  # type: ignore[import-not-found]
...
def _wants_color() -> bool:
    if "NO_COLOR" in os.environ:
        return False
    return os.environ.get("SELFGRAPH_HUD_COLOR", "1") != "0"
...
def _resolve_vault() -> Path:
    explicit = os.environ.get("SELFGRAPH_VAULT")
    if explicit:
        return Path(explicit).expanduser()
    return Path.home() / ".selfgraph"
...
def _render() -> str:
    ...
    parts.append(_ansi(f"selfgraph {VERSION}", C_CYAN, color))

    # 2. Claude usage (best-effort)
    limit_5h = max(1, int(os.environ.get("SELFGRAPH_HUD_5H_LIMIT", DEFAULT_5H_LIMIT)))
    limit_wk = max(1, int(os.environ.get("SELFGRAPH_HUD_WK_LIMIT", DEFAULT_WK_LIMIT)))
