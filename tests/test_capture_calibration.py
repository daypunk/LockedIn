"""Calibration guard tests for lockedin-capture fixtures.
...
SKILL_DIR = REPO_ROOT / "plugins" / "lockedin" / "skills" / "lockedin-capture"
BANNED_PHRASES_PATH = SKILL_DIR / "banned_phrases.json"
RUBRIC_PATH = SKILL_DIR / "RUBRIC.md"
PASS_DIR = REPO_ROOT / "tests" / "fixtures" / "capture" / "pass"
FAIL_DIR = REPO_ROOT / "tests" / "fixtures" / "capture" / "fail"

_REQUIRED_FRONTMATTER_KEYS = {"fixture_kind", "scenario_summary"}
...
def test_pass_fixtures_free_of_high_severity_banned_phrases() -> None:
    """Pass fixtures must not contain any high-severity banned phrase in body text."""
    phrases = _load_banned_phrases()
...
def test_rubric_exists_and_has_five_dimensions() -> None:
    """RUBRIC.md must exist, be non-empty, and list the five required dimensions."""
    ...
    missing = [d for d in required_dimensions if d not in content]
    assert not missing, (
        f"RUBRIC.md is missing the following dimension headings: {missing}"
    )


def test_pass_fixtures_have_scenario_summary() -> None:
    """Every pass fixture must have a scenario_summary field in frontmatter."""
    ...
    for fixture in pass_files:
        ...
        if not front_text.strip():
            missing_summary.append(f"{fixture.name}: missing YAML frontmatter")
            continue
        try:
            data = yaml.safe_load(front_text)
        except yaml.YAMLError as exc:
            missing_summary.append(f"{fixture.name}: YAML parse error — {exc}")
            continue
        if not isinstance(data, dict):
            missing_summary.append(f"{fixture.name}: frontmatter is not a YAML mapping")
            continue
        if not data.get("scenario_summary", "").strip():
            missing_summary.append(
                f"{fixture.name}: scenario_summary is missing or empty"
            )

    assert not missing_summary, (
        "Pass fixtures missing scenario_summary:\n" + "\n".join(missing_summary)
    )


def test_fixtures_have_valid_frontmatter() -> None:
    """All fixtures must have YAML frontmatter with fixture_kind and scenario_summary."""
    yaml = pytest.importorskip("yaml")
