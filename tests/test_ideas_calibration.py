"""Calibration guard tests for render-ideas fixtures.
...
SKILL_DIR = REPO_ROOT / "plugins" / "lockedin" / "skills" / "lockedin-render-ideas"
BANNED_PHRASES_PATH = SKILL_DIR / "banned_phrases.json"
PASS_DIR = REPO_ROOT / "tests" / "fixtures" / "ideas" / "pass"
FAIL_DIR = REPO_ROOT / "tests" / "fixtures" / "ideas" / "fail"
...
def _build_pattern(phrase: str) -> re.Pattern:
    ...
    Comparison-shortcut phrases (ending in 'for') use a prefix match
    to catch 'the Netflix of X' without requiring a right word boundary.
    """
    escaped = re.escape(phrase)
    if phrase.endswith(" for"):
