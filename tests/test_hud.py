"""Tests for the selfgraph hud statusLine command."""
...
def _isolated_env(monkeypatch, claude_dir: Path) -> None:
    ...
    monkeypatch.delenv("SELFGRAPH_HUD_COLOR", raising=False)
    monkeypatch.delenv("SELFGRAPH_HUD_5H_LIMIT", raising=False)
    monkeypatch.delenv("SELFGRAPH_HUD_WK_LIMIT", raising=False)


def test_hud_no_vault_emits_service_label(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.setenv("SELFGRAPH_VAULT", str(tmp_path / "no-such-vault"))
    _isolated_env(monkeypatch, tmp_path / "claude")
    rc = hud(["--no-color"])
    assert rc == 0
    out = capsys.readouterr().out.strip()
    assert out.startswith("selfgraph")
    assert "0.0.1" in out  # version surfaces
